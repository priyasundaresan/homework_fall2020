...
class FFModel(nn.Module, BaseModel):
    ...
    def forward(
        ...
        # FIXED(Q1) compute delta_pred_normalized and next_obs_pred
        # Hint: as described in the PDF, the output of the network is the
        # *normalized change* in state, i.e. normalized(s_t+1 - s_t).
        delta = self.delta_network(concatenated_input)
        #delta_pred_normalized = normalize(delta, delta_mean, delta_std)
        delta_pred_normalized = delta
        next_obs_pred = obs_unnormalized + unnormalize(delta, delta_mean, delta_std)
    ...
    def get_prediction(self, obs, acs, data_statistics):
        ...
        """
        outputs = self(ptu.from_numpy(obs), \
                          ptu.from_numpy(acs), \
        ...
        # FIXED(Q1) get numpy array of the predicted next-states (s_t+1)
    ...
    def update(self, observations, actions, next_observations, data_statistics):
        ...
        outputs = self(ptu.from_numpy(observations), \
                          ...
                          ptu.from_numpy(data_statistics["delta_mean"]), \
                          ptu.from_numpy(data_statistics["delta_std"]))
        delta_pred = outputs[1]

        loss = self.loss(target, delta_pred)
        #loss = # FIXED(Q1) compute the loss
        # Hint: `self(...)` returns a tuple, but you only need to use one of the
        # outputs.
